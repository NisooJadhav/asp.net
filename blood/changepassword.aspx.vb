...
Partial Class Default2
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Dim cmd As New SqlCommand("select count(username) from blood where username= '" + Session("un") + "' and password=@password", cn)
        cmd.Parameters.AddWithValue("@password", TextBox1.Text)
        cn.Open()
        Dim chk As Integer = cmd.ExecuteScalar
        cn.Close()
        If chk > 0 Then
            Dim cmdx As New SqlCommand("update blood set password='" + TextBox3.Text + "'where username='" + Session("un") + "'", cn)
            cn.Open()
